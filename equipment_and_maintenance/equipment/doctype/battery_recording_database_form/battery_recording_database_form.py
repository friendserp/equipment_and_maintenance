...
class BatteryRecordingDatabaseForm(Document):
	...
	def auto_set_reference_numbers(self):
		...
		# Set reference numbers for old battery items
		if self.old_battery_items:
			self._set_ref_numbers(self.old_battery_items)
		
		# Set reference numbers for new battery items
		if self.new_battery_items:
			self._set_ref_numbers(self.new_battery_items)
